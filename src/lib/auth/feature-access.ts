...
type UserWithOverrides = Pick<User, "id" | "role" | "fullName" | "isBlocked" | "blockedReason"> & {
...
function getFeatureDefinition(featureKey: FeatureKey) {
...
function getDefaultFeatureAccess(role: string, definition: FeatureDefinition): ResolvedFeatureAccess {
  if (role === "ADMIN") {
    return {
      ...definition,
      isEnabled: true,
