...
export type AdaptiveQuestionInsight = {
    topic: string;
    accuracy: number;
...
function calcXP(correct: number, total: number, streak: number, avgTime: number) {
    ...
    const accuracyBonus = correct / total >= 0.8 ? 20 : 0;
    const streakBonus = streak > 0 ? Math.min(streak * 2, 30) : 0;
    const speedBonus = avgTime < 45 ? 10 : 0;
...
function normalizeQuestionResults(questionResults: QuestionResult[]) {
...
export async function saveLearningProgressForExam(
    studentId: string,
    examId: string,
    attemptId: string,
    ...
    const normalizedResults = normalizeQuestionResults(questionResults);
    ...
    return withSerializableTransaction(async (tx) => {
        ...
        let profile = await tx.studentLearningProfile.findUnique({
        ...
        if (existingEvent && profile) {
            const weakTopics = Array.from(new Set(
                normalizedResults.filter((result) => !result.isCorrect).map((result) => result.topic),
            ));
...
export async function getAdaptiveQuestionInsights(
    studentId: string,
    subject: string,
    limit = 10,
): Promise<AdaptiveQuestionInsight[]> {
    ...
    return weakTopics.map((topicProgress) => ({
        topic: topicProgress.topic,
        accuracy: Math.round(topicProgress.accuracy * 100),
        dueForReview: topicProgress.nextReviewAt <= new Date(),
    }));
}
