    ...
    return (
        <div className="student-theme flex min-h-screen bg-[var(--student-bg)] text-[var(--student-text)]">
            <aside className="flex h-screen w-72 flex-col border-r border-[var(--student-border)] bg-[var(--student-ink)] text-white shadow-[20px_0_50px_rgba(24,31,34,0.08)]">
                <div className="border-b border-[var(--student-ink-border)] p-8">
                    <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--student-ink-border)] shadow-[0_14px_28px_rgba(31,92,80,0.12)]">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-[var(--student-muted)]">Admin Panel</p>
                        </div>
                    </div>
                </div>
                ...
                <nav className="mt-8 flex-1 space-y-2.5 px-6">
                    {navigation.map((item) => (
                        <Link
                            key={item.href}
                            href={item.href}
                            className="group relative flex items-center gap-4 rounded-[20px] px-6 py-4 text-[var(--student-border)] transition-all hover:bg-[var(--student-ink-hover)] hover:text-white active:scale-95"
                        >
                            <item.icon size={22} weight="bold" className="transition-colors group-hover:text-[var(--student-accent)]" />
                            <span className="text-[10px] font-black uppercase tracking-[0.2em]">{item.label}</span>
                        </Link>
                    ))}

                    <div className="mt-8 border-t border-[var(--student-ink-border)] pt-8">
                        <Link
                            href="/student/dashboard"
                            className="group flex items-center gap-4 rounded-[20px] px-6 py-4 text-[var(--student-border)] transition-all hover:bg-[var(--student-ink-hover)] hover:text-[var(--student-accent)] active:scale-95"
                        >
                ...
                <div className="mt-auto border-t border-[var(--student-ink-border)] p-6">
                    <div className="rounded-[24px] border border-[var(--student-ink-border)] bg-[var(--student-ink-hover)] p-5">
                        <div className="flex items-center gap-4">
                            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--student-accent-strong)] text-sm font-black shadow-[0_18px_30px_rgba(31,92,80,0.1)]">
                                {initials}
                            </div>
                            <div className="min-w-0 flex-1">
                                <p className="truncate text-xs font-bold uppercase tracking-wider text-white">{admin.fullName}</p>
                                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[var(--student-muted)]">Super Admin</p>
                            </div>
                        </div>
                        <Link
                            href="/auth/login"
                            className="mt-4 flex w-full items-center justify-center gap-2 rounded-[12px] border border-[var(--student-ink-border)] bg-[var(--student-ink-hover)] py-3.5 text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:bg-rose-500/10 hover:text-rose-300"
                        >
            ...
            <main className="student-shell flex flex-1 flex-col">
                <header className="sticky top-0 z-40 flex h-24 items-center justify-between border-b border-[var(--student-border)] bg-white/80 px-12 backdrop-blur-xl">
