...
export default function DashboardOverview() {
    ...
    if (!data) return (
        <div className="h-[400px] flex flex-col items-center justify-center text-center p-8 bg-white rounded-lg border border-dashed border-slate-200">
    ...
    return (
        <div className="space-y-6 pb-10 w-full max-w-[1280px] mx-auto   animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 px-1 relative">
                ...
                {STATS.map((stat) => (
                    <div key={stat.label} className="bg-white/80 backdrop-blur-md border border-slate-100 shadow-sm rounded-lg p-5 xl:p-6 relative overflow-hidden transition-all duration-300 hover:shadow-md hover:border-indigo-100/50 group flex flex-col justify-between min-h-[140px]">
                        <div className="flex items-start justify-between mb-4">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest  ">
                                {stat.label}
                            </span>
                            <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 shadow-sm border border-slate-100/50 group-hover:border-indigo-200 group-hover:scale-110 group-hover:shadow-indigo-600/10", stat.bg, stat.color)}>
            ...
            <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 px-1">
                {QUICK_ACTIONS.map((action) => (
                    <Link href={action.href} key={action.label} className={cn(
                        "flex items-center gap-4 p-4 rounded-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-lg active:scale-95 group",
                        action.color
                    )}>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 pt-4">
                ...
                <div className="lg:col-span-2 space-y-8">
                    {/* Performance Chart */}
                    <div className="bg-white/80 backdrop-blur-md p-6 rounded-lg border border-slate-100 shadow-sm">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <Link href="/teacher/updates" className="px-4 py-2 rounded-lg bg-slate-50 text-slate-600 text-[10px] font-bold uppercase tracking-widest hover:bg-slate-950 hover:text-white transition-all duration-200 border border-slate-100 active:scale-95 shadow-sm">View All</Link>
                        </div>

                        <div className="space-y-4">
                            {data.recentAnnouncements.length === 0 ? (
                                <div className="p-12 bg-white rounded-lg border border-slate-100 text-center shadow-sm border-dashed border-2">
                                ...
                                data.recentAnnouncements.map((ann) => (
                                    <div key={ann.id} className="p-6 bg-white rounded-lg border border-slate-100 hover:border-indigo-100 hover:shadow-md transition-all duration-300 flex gap-5 group relative overflow-hidden">
                                        <div className="w-10 h-10 rounded-lg bg-slate-50 shrink-0 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-sm border border-slate-100">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-lg border border-slate-100 shadow-sm">
                        {data.topStudents.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {data.topStudents.map((student, i) => (
                                    <div key={student.id} className="flex items-center justify-between p-4 rounded-lg transition-all duration-300 hover:bg-slate-50 group">
                                        <div className="flex items-center gap-4">
                                            <div className={cn("text-xs font-bold w-5 text-center", i === 0 ? "text-amber-500" : "text-slate-400")}>
                                                {student.rank}
                                            </div>
                                            <div className="w-10 h-10 rounded-lg bg-white border border-slate-100 flex items-center justify-center text-xs font-bold uppercase shadow-sm group-hover:border-indigo-200 transition-colors">
                        ...
                        <Link href="/teacher/students"
                            className="block w-full text-center py-3.5 mt-6 text-[10px] font-bold tracking-widest uppercase text-slate-400 hover:text-slate-950 transition-all border border-slate-100 rounded-lg hover:bg-slate-50 active:scale-[0.98]">
                            View All Students
                        </Link>
                    </div>

                    {/* Academy Insight Card */}
                    <div className="student-surface-dark group relative flex flex-col justify-between overflow-hidden rounded-lg p-6 transition-all duration-300 hover:shadow-[0_28px_44px_rgba(24,31,34,0.18)]">
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <button className="w-full py-3 rounded-lg bg-white text-slate-950 text-[10px] font-bold uppercase tracking-widest hover:bg-amber-400 transition-all active:scale-95">Analyze Growth</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="pt-8">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <Link href="/teacher/free-resources/materials" className="px-5 py-2.5 rounded-lg bg-slate-50 text-slate-600 text-[10px] font-bold uppercase tracking-widest hover:bg-slate-950 hover:text-white transition-all duration-200 border border-slate-100 shadow-sm active:scale-95">Manage Library</Link>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 px-1">
                    {data.recentMaterials.length === 0 ? (
                        <div className="lg:col-span-4 p-12 text-center text-slate-300 bg-white rounded-lg border-2 border-slate-100 border-dashed uppercase tracking-widest text-[10px] font-bold shadow-sm">
                            No materials uploaded yet
                        </div>
                    ) : (
                        data.recentMaterials.map((res) => (
                            <Link href="/teacher/free-resources/materials" key={res.id}
                                className="p-6 bg-white rounded-lg border border-slate-100 hover:border-indigo-100 hover:shadow-md transition-all duration-300 flex flex-col justify-between group cursor-pointer min-h-[140px] relative overflow-hidden">
                                <div className="flex items-start justify-between mb-4 relative z-10">
                                    <div className="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center shrink-0 border border-slate-100 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-sm">
